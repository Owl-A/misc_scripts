...
# encrypt_id=sys.argv[4]
form = cgi.FieldStorage()
a = form.getvalue('message')
b= form.getvalue('path')
c=form.getvalue('password')
d=form.getvalue('Id')
...
def inpute(message,image_path,password,encrypt_id):
    img=Image.open(image_path)
    convert(img)
    enc_data=encrypt(message,password)
    im = Image.open("h.bmp")
    im2=copy.copy(im)
    mount(im2,enc_data).save(encrypt_id+".bmp")
def decode(im):
    (j,k)=(im.size)
    pix=im.load()
    (R,G,B)=pix[j-1,k-1]
...
# print 'To Retrieve your password press 1\npress2'
# x=raw_input()
# if x=='1':
#     p=raw_input('Enter Your Password\n')
#     im3 = Image.open(encrypt_id+".bmp")
#     r=decrypt(decode(im3),p)
#     print(r)
# else:
#     exit()
